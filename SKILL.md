...
在已有经营驾驶舱中替换或嵌入网页汇报模式，同时要求驾驶舱保持不变。
...
不要把 `body`、`*`、`.slide`、`.active`、`.progress`、`.kpi`、`.container` 等全局选择器直接粘贴进驾驶舱。应使用唯一根节点（如 `.report-presenter`），并为汇报页面的幻灯片、按钮和显示状态加前缀（如 `pr-`），避免汇报模式改坏驾驶舱的卡片、间距或可见性。
...
8. **命名空间隔离** — 汇报样式全部加 `pr-` 前缀，不与驾驶舱冲突
9. **同一数据口径** — 驾驶舱和汇报模式共用期间、单位、主题名称和源数据
